...
# Component 1
n1    <- nsim*exp(fit1_all[["fit"]][["zero"]][["Nelder-Mead"]][["coef"]]["Comp1_delta_(Intercept)"])/(1 + exp(fit1_all[["fit"]][["zero"]][["Nelder-Mead"]][["coef"]]["Comp1_delta_(Intercept)"]))
mean1 <- fit1_all[["fit"]][["zero"]][["Nelder-Mead"]][["coef"]]["Comp1_beta_(Intercept)"] +
  hr*fit1_all[["fit"]][["zero"]][["Nelder-Mead"]][["coef"]]["Comp1_beta_hr"]
sd1   <- exp(fit1_all[["fit"]][["zero"]][["Nelder-Mead"]][["coef"]]["Comp1_lnsigma"])

# Component 2
n2    <- nsim*(1 - exp(fit1_all[["fit"]][["zero"]][["Nelder-Mead"]][["coef"]]["Comp1_delta_(Intercept)"]) /
  (1 + exp(fit1_all[["fit"]][["zero"]][["Nelder-Mead"]][["coef"]]["Comp1_delta_(Intercept)"])))
mean2 <- fit1_all[["fit"]][["zero"]][["Nelder-Mead"]][["coef"]]["Comp2_beta_(Intercept)"] +
  hr*fit1_all[["fit"]][["zero"]][["Nelder-Mead"]][["coef"]]["Comp2_beta_hr"]
sd2   <- exp(fit1_all[["fit"]][["zero"]][["Nelder-Mead"]][["coef"]]["Comp2_lnsigma"])
...
# Component 1
n1    <- nsim*exp(fit1_all[["fit"]][["zero"]][["nlminb"]][["coef"]]["Comp1_delta_(Intercept)"])/(1 + exp(fit1_all[["fit"]][["zero"]][["nlminb"]][["coef"]]["Comp1_delta_(Intercept)"]))
mean1 <- fit1_all[["fit"]][["zero"]][["nlminb"]][["coef"]]["Comp1_beta_(Intercept)"] +
  hr*fit1_all[["fit"]][["zero"]][["nlminb"]][["coef"]]["Comp1_beta_hr"]
sd1   <- exp(fit1_all[["fit"]][["zero"]][["nlminb"]][["coef"]]["Comp1_lnsigma"])

# Component 2
n2    <- nsim*(1 - exp(fit1_all[["fit"]][["zero"]][["nlminb"]][["coef"]]["Comp1_delta_(Intercept)"]) /
  (1 + exp(fit1_all[["fit"]][["zero"]][["nlminb"]][["coef"]]["Comp1_delta_(Intercept)"])))
mean2 <- fit1_all[["fit"]][["zero"]][["nlminb"]][["coef"]]["Comp2_beta_(Intercept)"] +
  hr*fit1_all[["fit"]][["zero"]][["nlminb"]][["coef"]]["Comp2_beta_hr"]
sd2   <- exp(fit1_all[["fit"]][["zero"]][["nlminb"]][["coef"]]["Comp2_lnsigma"])
...
# Component 1
n1    <- nsim*exp(fit1_all[["fit"]][["zero"]][["hjn"]][["coef"]]["Comp1_delta_(Intercept)"])/(1 + exp(fit1_all[["fit"]][["zero"]][["hjn"]][["coef"]]["Comp1_delta_(Intercept)"]))
mean1 <- fit1_all[["fit"]][["zero"]][["hjn"]][["coef"]]["Comp1_beta_(Intercept)"] +
  hr*fit1_all[["fit"]][["zero"]][["hjn"]][["coef"]]["Comp1_beta_hr"]
sd1   <- exp(fit1_all[["fit"]][["zero"]][["hjn"]][["coef"]]["Comp1_lnsigma"])

# Component 2
n2    <- nsim*(1 - exp(fit1_all[["fit"]][["zero"]][["hjn"]][["coef"]]["Comp1_delta_(Intercept)"]) /
  (1 + exp(fit1_all[["fit"]][["zero"]][["hjn"]][["coef"]]["Comp1_delta_(Intercept)"])))
mean2 <- fit1_all[["fit"]][["zero"]][["hjn"]][["coef"]]["Comp2_beta_(Intercept)"] +
  hr*fit1_all[["fit"]][["zero"]][["hjn"]][["coef"]]["Comp2_beta_hr"]
sd2   <- exp(fit1_all[["fit"]][["zero"]][["hjn"]][["coef"]]["Comp2_lnsigma"])
...
To validate the R implementation of adjusted limited dependent variable mixture models, we estimate the four models presented in @HernandezAlava2015 as reference models and compare the results to the results in STATA\textsuperscript{\textregistered}.\footnote{The STATA\textsuperscript{\textregistered} and R code for model estimation is included in the appendix.}
...
The parameter estimates and standard errors obtained in R are very similar to the results of the STATA\textsuperscript{\textregistered} package (table \@ref(tab:tab-comp-stata) and table \@ref(tab:tab-compse-stata)). The coefficient of the Oxford Hip Score in component 2 of model 2 differs between R and STATA\textsuperscript{\textregistered}, and the R package does not return standard errors of parameters. However, model 2 uses quite extreme settings. Although models 1 and 3 yield different parameter estimates, they converged at the same log-likelihood. This further supports the hypothesis of multiple local optima of the likelihood. The log-likelihood is consistently smaller in R than in STATA\textsuperscript{\textregistered}, but the relative ordering of models is consistent across the two implementations. 

The comparison the R and STATA\textsuperscript{\textregistered} packages showed that the R implementation sometimes behaves differently than the STATA\textsuperscript{\textregistered} package, but the results are not indicative of technical errors in the R implementation of adjusted limited dependent variable mixture models.
...
Fitted values show very similar marginal distributions on both platforms (figure \@ref(fig:box-pred-yhat-stata)). R does not return fitted values in model 2 due to non-finite predictions for all observations with the obtained parameter estimates. The summary statistics of differences between R and STATA\textsuperscript{\textregistered} in fitted values for all observations also suggests that individual predictions are quite similar across platforms (table \@ref(tab:compyhat).

Standard errors of fitted values differ more between platforms than fitted values (figure \@ref(fig:box-pred-se-stata) and table \@ref(tab:compse). The difference is particularly pronounced in model 1, but the standard errors from STATA\textsuperscript{\textregistered} seem quite extreme compared to all other reference models. R does not return standard errors of fitted values in model 2.

```{r load-r-stata, echo = FALSE, results = "asis"}
...
```{r box-pred-yhat-stata, echo = FALSE, results = "asis", warning = FALSE, fig.height = 2.9, fig.width = 6, fig.cap = 'Fitted values in R and STATA'}

stata_yhat_long <- stata_yhat
names(stata_yhat_long) <- c("Model 1", "Model 2", "Model 3", "Model 4")
...
r_yhat_long <- rbind(cbind(variable = "Model 1",
                           value = fit1_nlminb[["pred"]][["yhat"]],
                           software = "R"),
                     cbind(variable = "Model 2",
                           value = fit1_cstr_stata[["pred"]][["yhat"]],
                           software = "R"),
                     cbind(variable = "Model 3",
                           value = fit1_cons[["pred"]][["yhat"]],
                           software = "R"),
                     cbind(variable = "Model 4",
...
suppressWarnings(
  ggplot(plotdf, aes(x = variable, y = as.numeric(value), fill = software)) +
    geom_boxplot() +
    ylab("Fitted values") +
    ggplot_theme +
...
```{r box-pred-se-stata, echo = FALSE, results = "asis", warning = FALSE, fig.height = 2.9, fig.width = 6, fig.cap = 'Standard errors of fitted values in R and STATA'}

stata_se_long <- stata_se
names(stata_se_long) <- c("Model 1", "Model 2", "Model 3", "Model 4")
...
r_se_long <- rbind(cbind(variable = "Model 1",
...
```{r tab-pred-stata, echo = FALSE, results = "asis"}

# Extract fitted values in estimation data
sumhat <- list()

sumhat[["yhat"]][["(1)"]] <- summary(stata_yhat[, 1] - fit1_nlminb[["pred"]][["yhat"]])
sumhat[["yhat"]][["(2)"]] <- summary(stata_yhat[, 2] - fit1_cstr_stata[["pred"]][["yhat"]])
sumhat[["yhat"]][["(3)"]] <- summary(stata_yhat[, 3] - fit1_cons[["pred"]][["yhat"]])
sumhat[["yhat"]][["(4)"]] <- summary(stata_yhat[, 4] - fit2_stata[["pred"]][["yhat"]])

ymat <- suppressWarnings( round(do.call("cbind", sumhat[["yhat"]]), 6) )
ymat <- ymat[1:6, ]

print(xtable::xtable(ymat, 
                     align = paste0("l", 
                                    paste(rep("r", ncol(ymat)), 
                                          collapse = "")),
                     label = "tab:compyhat",
                     caption = "Summary statistics of differences of fitted values between R and STATA (positive values suggest larger values in STATA)",
                     digits = 6),
      type = "latex",
      include.rownames = TRUE,
      hline.after = c(-1, 0, nrow(ymat)),
      comment = FALSE,
      caption.placement = "top")


sumhat[["se"]][["(1)"]] <- summary(stata_se[, 1] - fit1_nlminb[["pred"]][["se.fit"]])
sumhat[["se"]][["(2)"]] <- summary(stata_se[, 2] - fit1_cstr_stata[["pred"]][["se.fit"]])
sumhat[["se"]][["(3)"]] <- summary(stata_se[, 3] - fit1_cons[["pred"]][["se.fit"]])
sumhat[["se"]][["(4)"]] <- summary(stata_se[, 4] - fit2_stata[["pred"]][["se.fit"]])

semat <- suppressWarnings( round(do.call("cbind", sumhat[["se"]]), 6) )
semat <- semat[1:6, ]

print(xtable::xtable(semat, 
                     align = paste0("l", 
                                    paste(rep("r", ncol(semat)), 
                                          collapse = "")),
                     label = "tab:compse",
                     caption = "Summary statistics of differences of standard errors of fitted values between R and STATA (positive values suggest larger values in STATA)",
                     digits = 6),
      type = "latex",
      include.rownames = TRUE,
      hline.after = c(-1, 0, nrow(semat)),
      comment = FALSE,
      caption.placement = "top")
...
\newpage
## R code for the estimation of reference models
...
\newpage
## STATA\textsuperscript{\textregistered} code for the estimation of reference models

```{r stata_code, echo = TRUE, eval = FALSE}
* (1) Model 1
aldvmm eq5d hr, ncomponents(2)

* (2) Model 1 with constraints
matrix a = e(b)
...
* (3) Model 1 initital values from constant-only model
aldvmm eq5d hr, ncomp(2) inim(cons)

* (4) Model 2 user-defined initial values
matrix start2lc=e(b)
